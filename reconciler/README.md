...
| PR 9 (this PR) | Flip default. `--experimental` becomes no-op. New `docs/adoption/`. README + setup updated. |
...
uv run --project reconciler --group dev magpie-reconciler plan \
  --intent path/to/.apache-magpie.intent.yaml \
  --lock path/to/.apache-magpie.lock
...
# Add --render-templates-to <adopter>/.apache-magpie-overrides/rendered
