...
- the adopter intent from ``--intent`` (default
  ``.apache-magpie.intent.yaml`` in cwd);
- the current lock (if present) from ``--lock`` (default
  ``.apache-magpie.lock`` in cwd).
...
def main(argv: list[str] | None = None) -> int:
    ...
    plan.add_argument(
        "--intent",
        default=".apache-magpie.intent.yaml",
        help="Path to intent file (default: .apache-magpie.intent.yaml in cwd)",
    )
    plan.add_argument(
        "--lock",
        default=".apache-magpie.lock",
        help="Path to current lock (default: .apache-magpie.lock in cwd)",
    )
    plan.add_argument(
        "--framework-root",
        type=Path,
