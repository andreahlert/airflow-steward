...
| [`intent.schema.json`](intent.schema.json) | `.apache-magpie.intent.yaml` in adopter repo | Adopter PMC | Before every `magpie plan` |
| [`lock.schema.json`](lock.schema.json) | `.apache-magpie.lock` in adopter repo | Reconciler | Before adopter tooling reads it |
