...
PR 9 extended `setup` SKILL.md description to advertise Model C. The combined frontmatter now exceeds the 1536-character truncation limit Claude Code enforces.
...
Reproduced empirically: this fails with `No module named reconciler` because the reconciler package lives at `<adopter>/.apache-magpie/reconciler/` (gitignored snapshot) and Python does not look there by default.
...
cd .apache-magpie
uv run ... python -m reconciler plan --intent ../.apache-magpie.intent.yaml --lock ../.apache-magpie.lock
# Or set PYTHONPATH
PYTHONPATH=.apache-magpie uv run ... python -m reconciler plan
...
### 5. setup description/body mismatch

`setup/SKILL.md` frontmatter description was changed to lead with Model C. The body of the same file (untouched) still walks the agent through the legacy family-based takeover.
...
### 7. setup manifest version bump

`setup/manifest.yaml`: 1.0.0 → 2.0.0. Justifiable per semver (recommended flow changed), but it propagates into `registry/skills-index.json` and any adopter pinning the skill. Not pure file reorganization.
...
`projects/_example-airflow/.apache-magpie.lock` is generated with `--stable-timestamp`. It is deterministic across runs **on the same machine with the same PyYAML version**. PyYAML serialisation of `null` and edge-case strings has changed between releases. The current checksum is `sha256:c561eee5...`; a future PyYAML upgrade could produce a different lock without any intent change. Not exercised in CI.

## Summary

| # | Severity | Item | Status (this PR) |
|---|---|---|---|
| 1 | P0 | skill-validator pytest broken on main (setup description over 1536 chars) | **fixed**, description shrunk to 1197 chars + 331 when_to_use = 1528 (< 1536); `pytest -k test_real_repo_passes` passes |
...
| 4 | P1 | `python -m reconciler` from adopter cwd does not resolve module | **fixed (final)**, reconciler now ships `pyproject.toml` + `uv.lock` (src-layout, console-script `magpie-reconciler`); adopter command is `uv run --project .apache-magpie/reconciler magpie-reconciler plan`. PYTHONPATH workaround from the first cleanup PR removed. |
| 5 | P1 | setup description advertises a flow the body does not implement | **fixed**, description reverted to a neutral one-liner pointing at `docs/adoption/`; body remains the legacy adopter flow which matches |
...
| 7 | P2 | setup version 1.0.0 → 2.0.0 | accepted, semver justified |
...
- a description/body mismatch on `setup`.
