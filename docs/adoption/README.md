...
Adoption under the **intent + lock** model. As of PR 9 this is the default flow. The legacy family-based `setup` takeover remains available as a fallback during the transition window; documentation is being migrated.
...
2. **Author your intent.** Copy [`projects/_example-airflow/.apache-magpie.intent.yaml`](../../projects/_example-airflow/.apache-magpie.intent.yaml) to your repo root as `.apache-magpie.intent.yaml`. Edit four blocks: `capabilities`, `overrides.exclude`, `overrides.pin`, `overrides.params`.
...
uv run --project .apache-magpie/reconciler magpie-reconciler plan

# Materialise: write lock + symlinks + render templates
uv run --project .apache-magpie/reconciler magpie-reconciler apply \
  --symlink-target .claude/skills \
  --render-templates-to .apache-magpie-overrides/rendered
```

Commit:
- `.apache-magpie.intent.yaml`
- `.apache-magpie.lock`
- `.apache-magpie-overrides/` (any rendered templates you want versioned)

Do not commit `.apache-magpie/` (gitignored snapshot) or the symlinks under `.claude/skills/` (also gitignored).
